      ...
      if (name && email && password) {
        ...
        if (isUser) {
          ...
          if (resUser) {
            return res
              .status(200)
              .json({ message: "Successfully Register", user: resUser });
          }
        }
      } else {
      ...
      if (email && password) {
        ...
        if (isUser) {
          if (
            ...
            //generate token
            const token = jwt.sign({ userID: isUser._id }, "aayush", {
              expiresIn: "2d",
            });
  ...
  static forgotPassword = async (req, res) => {
    ...
    try {
      if (email) {
        ...
        if (isUser) {
          // generate token
          const secretKey = isUser._id + "aayush";
...
<body marginheight="0" topmargin="0" marginwidth="0" style="margin: 0px; background-color: #f2f3f8;" leftmargin="0">
  ...
  static forgotPasswordEmail = async (req, res) => {
    const { newPassword, confirmPassword } = req.body;
        ...
        if (newPassword === confirmPassword) {
          ...
          if (isValid) {
            ...
            if (isSuccess) {
              ...
              return res
                .status(400)
                .json({ message: "Sorry, Password changed Unsuccessfully" });
            }
          } else {
          ...
          return res
            .status(400)
            .json({ message: "password andconfirm password not matched" });
        }
      } else {
